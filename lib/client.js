window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    if (document.querySelector('style[data-plugin-css="dsh-multimedia-webui-input"]') === null) {
      const style = document.createElement('style');
      style.dataset.plugin = 'dsh-multimedia-webui-input';
      style.dataset.pluginCss = 'dsh-multimedia-webui-input';
    ...
    function AttachmentDock(props) {
      useRevision();
      // Blank sessions use the composer overlay so the attachment cell stays
      // attached to the input.  Do not also emit the normal dock: another
      // plugin may contribute an entry between these two slot surfaces, which
      // makes a DOM-adjacency CSS guard unreliable.
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => inputTriggers.registerSource(source), 'community-multimedia-webui-input: reference codec');
