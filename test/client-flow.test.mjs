...
test('selection stays browser-local until the DSH reference serializer sends it', async () => {
  ...
  try {
    ...
    assert.equal(handoff.id, 'dsh-multimedia-webui-input');
    const plugin = handoff.factory(specifier => {
      if (specifier === 'react') return fakeReact();
      throw new Error(`unexpected client require ${specifier}`);
    });
    assert.deepEqual(plugin.inject, ['slots', 'conversation', 'sessions', 'inputTriggers']);
    ...
    assert.ok(dock);
    const blankDock = dock.component({
      input: snapshot,
      useSession: selector => selector({ blank: true }),
      remove() {},
    });
    assert.equal(blankDock, null,
      'blank sessions must render attachments only in the hero overlay');
    const activeDock = dock.component({
      input: snapshot,
      useSession: selector => selector({ blank: false }),
      remove() {},
    });
    assert.ok(activeDock,
      'non-blank sessions must keep the normal attachment dock');
    const hero = registrations.find(row => row.options.id === 'community-multimedia-webui-input-hero-dock');
    assert.ok(hero);
    const blankHero = hero.component({
      useSession: selector => selector({ blank: true }),
      useInput: selector => selector(snapshot),
      remove() {},
    });
    assert.equal(blankHero.props.className, 'dshca-hero-dock',
      'blank sessions must keep exactly one hero attachment surface');
    const activeHero = hero.component({
      useSession: selector => selector({ blank: false }),
      useInput: selector => selector(snapshot),
      remove() {},
    });
    assert.equal(activeHero, null,
      'non-blank sessions must not render the hero attachment surface');
