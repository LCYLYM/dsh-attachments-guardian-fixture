...
test('package exposes one client contract to current and legacy DSH scanners', async () => {
  ...
  assert.ok(!manifest.dsh.client.inject.includes('@deepseek-ai/dsh-client-ui-slash'));
...
async function fakeDsh(root) {
  ...
  await writeFile(executable, "#!/bin/sh\nprintf \"name: 'dsh-multimedia-webui-input'\\n\"\n");
...
import { join } from 'node:path';
const name = 'dsh-multimedia-webui-input';
...
if (args.includes('--dump-config')) {
  ...
  if (bundles.includes(name)) process.stdout.write("name: 'dsh-multimedia-webui-input'\\n");
  process.exit(0);
}
process.exit(0);
`);
...
test('clone-local installer survives a staging switch through the stable source resolver root', async () => {
  ...
  try {
    ...
    const packageTarget = join(root, 'source', 'node_modules', 'dsh-multimedia-webui-input');
    const manifest = JSON.parse(await readFile(join(packageTarget, 'package.json'), 'utf8'));
    assert.equal(manifest.name, 'dsh-multimedia-webui-input');
...
test('renames a legacy attachment install without leaving two active packages', async () => {
  ...
  try {
    ...
    const installed = await run(['install'], env);
    assert.equal(installed.code, 0, installed.stderr);
    const renamedPackage = join(root, 'source', 'node_modules', 'dsh-multimedia-webui-input');
    assert.equal(JSON.parse(await readFile(join(renamedPackage, 'package.json'), 'utf8')).name, 'dsh-multimedia-webui-input');
...
test('installs through the native profile bundle layer and uninstalls reversibly', async () => {
  ...
  try {
    ...
    assert.match(profileManifest.dependencies['dsh-multimedia-webui-input'], /^link:/);
    assert.ok(profileManifest.dsh.profile.bundles.includes('dsh-multimedia-webui-input'));
    ...
    assert.equal(removedManifest.dependencies['dsh-multimedia-webui-input'], undefined);
    assert.ok(!removedManifest.dsh.profile.bundles.includes('dsh-multimedia-webui-input'));
  } finally {
    await rm(root, { force: true, recursive: true });
  }
});
